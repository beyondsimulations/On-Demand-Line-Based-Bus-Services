...
plot_save_path = "plots/service_vs_fulfillment_comparison_$(results_version)_$(solver).png"
...
\\begin{tablenotes}
      ...
      \\item Positive gaps indicate potential for service improvement; negative gaps suggest over-promising.
\\end{tablenotes}
\\end{threeparttable}
\\end{table}
"""
...
# Create figure with both bar plot and violin plot
fig = Figure(size=(1200, 600))

# Create bar plot (left panel)
ax_bar = Axis(fig[1, 1],
    xlabel="Depot",
    ylabel="Service/Fulfillment Rate",
    title="O3.2: Mean Service vs. Actual Fulfillment",
    xticks=(1:length(depots), depots),
    xticklabelrotation=π/4
)

# Create violin plot (right panel)
ax_violin = Axis(fig[1, 2],
    xlabel="Depot",
    ylabel="",
    title="O3.2: Distribution of Daily Values",
    xticks=(1:length(depots), depots),
    xticklabelrotation=π/4
)
...
barplot!(ax_bar, x_positions .- 0.2, actual_rates, 
    color=(:lightblue, 0.6),
    strokecolor=:navy,
    strokewidth=1.5,
    width=0.35,
...
barplot!(ax_bar, x_positions .+ 0.2, service_levels, 
    color=(:orange, 0.6),
...
errorbars!(ax_bar, x_positions .- 0.2, actual_rates, plot_data.actual_fulfillment_std, 
    color=:navy, linewidth=2)

# Add error bars for service level standard deviation
errorbars!(ax_bar, x_positions .+ 0.2, service_levels, plot_data.achievable_service_std, 
    color=:darkorange, linewidth=2)

# --- ADD JITTERED SCATTER POINTS ---
# Add individual daily values as jittered scatter points for transparency
jitter_width = 0.15  # Width of jitter
...
for depot in depots
    ...
    if nrow(actual_daily) > 0
        ...
        scatter!(ax_bar, x_jitter, actual_daily.fulfillment_rate,
    ...
    if nrow(service_daily) > 0
        ...
        scatter!(ax_bar, x_jitter, service_daily.achieved_service_level,
...
# Prepare data for violin plots - we need the daily values, not just means
violin_data = DataFrame()

# Get daily fulfillment data
for depot in depots
    depot_fulfillment_daily = fulfillment_stats[fulfillment_stats.depot .== ("VLP " * depot), :]
    for row in eachrow(depot_fulfillment_daily)
        push!(violin_data, (
            depot = depot,
            value = row.fulfillment_rate,
            type = "Actual Fulfillment",
            depot_idx = findfirst(d -> d == depot, depots)
        ))
    end
end

# Get daily service level data
for depot in depots
    depot_service_daily = daily_service_levels[daily_service_levels.depot_name .== ("VLP " * depot), :]
    for row in eachrow(depot_service_daily)
        push!(violin_data, (
            depot = depot,
            value = row.achieved_service_level,
            type = "Service Achievement",
            depot_idx = findfirst(d -> d == depot, depots)
        ))
    end
end

# Create violin plots for each depot
for depot in depots
    depot_idx = findfirst(d -> d == depot, depots)
    
    # Actual fulfillment violin
    actual_data = violin_data[(violin_data.depot .== depot) .& (violin_data.type .== "Actual Fulfillment"), :]
    if nrow(actual_data) > 0
        violin!(ax_violin, fill(depot_idx - 0.2, nrow(actual_data)), actual_data.value,
            color=(:lightblue, 0.6),
            strokecolor=:navy,
            strokewidth=1.5,
            width=0.3,
    ...
    # Service achievement violin
    service_data = violin_data[(violin_data.depot .== depot) .& (violin_data.type .== "Service Achievement"), :]
    if nrow(service_data) > 0
        violin!(ax_violin, fill(depot_idx + 0.2, nrow(service_data)), service_data.value,
            color=(:orange, 0.6),
            strokecolor=:darkorange,
            strokewidth=1.5,
            width=0.3,
...
try
    ...
    @warn "PNG save failed, trying without px_per_unit: $e"
...
# Note: Violin plots may visually extend beyond data range due to kernel density estimation
# but the actual data is constrained to [0, 1] as verified above
