...
# Convert time columns to proper time handling
# The time columns are already of type Time, so we can use them directly
...
heatmap_data = zeros(Int, length(depots), 24)
...
for hour_idx in 1:24, depot_idx in 1:length(depots)
    ...
    if value > 0.0  # Only show values > 0 buses
...
println("\n" * "="^70)
println("FINAL CORRECT AVERAGE HEATMAP COMPLETE!")
println("="^70)
...
for hour_idx in 1:24, depot_idx in 1:length(depots)
    value = average_heatmap_data[depot_idx, hour_idx]
    if value > 0  # Only show values > 0 buses
        text_color = value > maximum(average_heatmap_data) * 0.6 ? :white : :black
        text!(ax_avg, hour_idx, depot_idx,
            text=string(round(value, digits=1)),
            align=(:center, :center),
            fontsize=9,
